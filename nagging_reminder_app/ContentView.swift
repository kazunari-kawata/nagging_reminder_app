...
struct ContentView: View {
    ...
    private var overdueTasksToday: [TaskItem] {
        taskManager.tasks.filter { taskManager.isApplicableToday($0) && !$0.isCompleted && isOverdue($0) }
    }
    ...
    private func tabBarItem(icon: String, label: String, index: Int, filled: Bool = false) -> some View {
        Button {
            selectedTab = index
        } label: {
            VStack(spacing: 4) {
