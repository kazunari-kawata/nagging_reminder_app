...
final class TaskManager {
    ...
    func addTask(name: String, schedule: RepeatSchedule, nagIntervalMinutes: Int, nagCount: Int, dueDate: Date?) {
        let task = TaskItem(name: name, repeatSchedule: schedule, nagIntervalMinutes: nagIntervalMinutes, nagCount: nagCount, dueDate: dueDate)
        tasks.append(task)
        let ids = buildAndScheduleNotifications(for: tasks[tasks.count - 1])
        tasks[tasks.count - 1].pendingNotificationIDs = ids
    }

    func updateTask(_ task: TaskItem, name: String, schedule: RepeatSchedule, nagIntervalMinutes: Int, nagCount: Int, dueDate: Date?) {
        guard let index = tasks.firstIndex(where: { $0.id == task.id }) else { return }
        cancelNotifications(for: tasks[index])
        tasks[index].name = name
        tasks[index].repeatSchedule = schedule
        tasks[index].nagIntervalMinutes = nagIntervalMinutes
        tasks[index].nagCount = nagCount
    ...
    private func buildAndScheduleNotifications(for task: TaskItem) -> [String] {
        ...
        case .weekdays(let time):
            for weekday in 2...6 {
                var comps = DateComponents()
                comps.weekday = weekday; comps.hour = time.hour; comps.minute = time.minute
                add(id: "\(base)_wd_\(weekday)",
                    trigger: UNCalendarNotificationTrigger(dateMatching: comps, repeats: true))
            }
            if let next = nextFireDate(for: task.repeatSchedule) { addNagChain(after: next) }
        ...
        case .monthly(let day, let time):
            var comps = DateComponents()
            comps.day = day; comps.hour = time.hour; comps.minute = time.minute
            add(id: "\(base)_0",
                trigger: UNCalendarNotificationTrigger(dateMatching: comps, repeats: true))
            if let next = nextFireDate(for: task.repeatSchedule) { addNagChain(after: next) }
        ...
        case .yearly(let month, let day, let time):
            var comps = DateComponents()
            comps.month = month; comps.day = day; comps.hour = time.hour; comps.minute = time.minute
            add(id: "\(base)_0",
                trigger: UNCalendarNotificationTrigger(dateMatching: comps, repeats: true))
            if let next = nextFireDate(for: task.repeatSchedule) { addNagChain(after: next) }
        }
    ...
    private func nextFireDate(for schedule: RepeatSchedule, after date: Date = Date()) -> Date? {
        ...
        case .weekdays(let time):
            ...
            var search = date
            for _ in 0..<14 {
                guard let candidate = cal.nextDate(after: search, matching: comps, matchingPolicy: .nextTime)
                else { break }
        ...
        case .weekly(let weekday, let time):
            var comps = DateComponents()
            comps.weekday = weekday; comps.hour = time.hour; comps.minute = time.minute; comps.second = 0
            return cal.nextDate(after: date, matching: comps, matchingPolicy: .nextTime)

        case .monthly(let day, let time):
            var comps = DateComponents()
            comps.day = day; comps.hour = time.hour; comps.minute = time.minute; comps.second = 0
            return cal.nextDate(after: date, matching: comps, matchingPolicy: .nextTime)
...
class NotificationDelegate: NSObject, UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(
        ...
        if response.actionIdentifier == "SNOOZE_1HR",
