...
final class TaskManager {
  ...
  func addTask(
    name: String, schedule: RepeatSchedule, nagIntervalMinutes: Int, nagCount: Int,
    isTimeSensitive: Bool, dueDate: Date?
  ) {
    let task = TaskItem(
      name: name, repeatSchedule: schedule, nagIntervalMinutes: nagIntervalMinutes,
      nagCount: nagCount, isTimeSensitive: isTimeSensitive, dueDate: dueDate)
    tasks.append(task)
  ...
  func updateTask(
    ...
    nagCount: Int, isTimeSensitive: Bool, dueDate: Date?
  ) {
    ...
    tasks[index].isTimeSensitive = isTimeSensitive
  ...
  private func buildAndScheduleNotifications(for task: TaskItem) -> [String] {
    ...
    func addNagChain(after baseDate: Date) {
      guard task.nagCount > 1, task.nagIntervalMinutes > 0 else { return }
      let intervalSeconds = TimeInterval(task.nagIntervalMinutes * 60)
      for i in 1...(task.nagCount - 1) {
