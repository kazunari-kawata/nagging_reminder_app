...
struct TaskItem: Identifiable {
  ...
  var nagCount: Int  // how many nag notifications to chain
  var isTimeSensitive: Bool  // deliver through Focus / DND via Time Sensitive interruption level
  var dueDate: Date?
...
extension TaskItem: Codable {
  enum CodingKeys: String, CodingKey {
    ...
    case nagIntervalMinutes, nagCount
    case isTimeSensitive = "isCriticalAlert"  // stored key kept for backward-compat with saved data
  ...
  func encode(to encoder: Encoder) throws {
    ...
    try c.encode(nagCount, forKey: .nagCount)
    try c.encode(isTimeSensitive, forKey: .isTimeSensitive)
  ...
  init(from decoder: Decoder) throws {
    ...
    nagCount = try c.decodeIfPresent(Int.self, forKey: .nagCount) ?? 3
    isTimeSensitive = try c.decodeIfPresent(Bool.self, forKey: .isTimeSensitive) ?? false
