...
@Observable final class TimerManager {
  ...
  private func insertDefaultPresetsIfNeeded() {
    guard !UserDefaults.standard.bool(forKey: "defaultTimerPresetsInserted") else { return }
    UserDefaults.standard.set(true, forKey: "defaultTimerPresetsInserted")
    presets = [
      TimerPreset(name: String(localized: "preset.timer.hiit"), totalSeconds: 20 * 60),
      TimerPreset(name: String(localized: "preset.timer.coffee.break"), totalSeconds: 15 * 60),
      TimerPreset(name: String(localized: "preset.timer.study"), totalSeconds: 25 * 60),
      TimerPreset(name: String(localized: "preset.timer.stretch"), totalSeconds: 5 * 60),
    ]
  }
}
...
struct TimerPresetSheet: View {
  ...
  var body: some View {
    NavigationStack {
        ...
        Section {
          HStack {
            ...
            Text(
              totalSeconds >= 3600
                ? String(
                  format: "%d:%02d:%02d", totalSeconds / 3600, (totalSeconds % 3600) / 60,
                  totalSeconds % 60)
      ...
      .navigationTitle(
        preset == nil ? String(localized: "New Timer") : String(localized: "Edit Timer")
      )
